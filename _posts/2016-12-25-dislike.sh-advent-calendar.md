...
### `LANG=en_US.UTF-8` 時の `[a-z]` パターンマッチ
...
なんと `c` から `z` の英字にもマッチします。これは仕様なんだそうです。
...
近年の Debian や Ubuntu であれば `C.UTF-8` というロケールが利用できるのえ、
`en_US.UTF-8` の代わりにこれを利用しましょう。ほかの環境では
bash 以外を利用しましょう。
...
回避策2。最初の位置パラメーターの数が 0 なら `shift` を実行しないようにします。
...
### シグナルで終了したコマンドの $? が 128+シグナル番号でない
...
ksh, mksh では `function` で関数を定義すると、関数内の `$0`
...
```console
$ zsh -c 'set -o no_FUNCTION_ARGZERO; function funcname { echo "$0"; };
funcname' progname argv1 argv2 argv3
```
...
* echo コマンドの違いと移植性の問題 - 拡張 POSIX シェルスクリプト Advent Calendar 2013 - ダメ出し Blog (再掲)
  * <https://fumiyas.github.io/2013/12/08/echo.sh-advent-calendar.html>

### 対話シェル時に `[!abc]` でヒストリー展開しようとする

パス名展開の文字クラスの否定は `[!...]` はですが、
...
あまり大きな問題ではありせん。
対話シェル時は POSIX sh とは非互換ですが `[^...]` を使いましょう。
