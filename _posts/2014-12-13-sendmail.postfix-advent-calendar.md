...
```console
$ echo -n にゃんぱすー |perl -MEncode -pe '$_ = encode("MIME-Header-ISO_2022_JP", decode("UTF-8",$_))'; echo
=?ISO-2022-JP?B?GyRCJEskYyRzJFEkOSE8GyhC?=
$ echo -n にゃんぱすー |perl -MEncode -pe '$_ = encode("MIME-Header", decode("UTF-8",$_))'; echo
=?UTF-8?B?44Gr44KD44KT44Gx44GZ44O8?=
```

Perl の場合、改行コードも含めて処理させた場合の結果が不安定なので、
改行コードは取り除いて処理することをお勧めします。
