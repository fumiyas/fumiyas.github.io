...
Postfix の `sendmail` コマンドを MUA として用い、メールを発信方法について紹介します。
Sendmail や qmail など、ほかの MTA の `sendmail` でも概ね同じです。
...
要は「ふつうのメールに見える」ようにすればいいだけです。
  ...
  * `To:`、`Cc:` (必須)
    ...
    * あるユーザー宛 1人だけに送信するのであれば、その人宛であることを明示するために、
...
現代の MTA と MUA であれば、Base64 などなしで、UTF-8
のまま本文に突っ込むことも可能です。
...
デフォルトで文字エンコーディング変換以外の変換処理がされるため、これを抑制します。
  ...
  * `-x`
    * JIS X 0201 カナ (いわゆる「半角カナ」) を JIS X 0208 カナに変換しません。
  * `-m0`
