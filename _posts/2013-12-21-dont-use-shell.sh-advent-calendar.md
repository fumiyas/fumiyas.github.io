...
ポイントはこれだけです。
  ...
  2. パラメーターは環境変数で渡す。
  3. ***エスケープ不要！***
     (実際は 1 のシェルスクリプトで環境変数を受け取る(変数展開)ときにダブルクォートしているので、間接的(?)にエスケープしていると言える)
...
Ruby の `system()`
のように、起動するコマンドの環境変数を別途指定できる実装であれば問題ありません。
